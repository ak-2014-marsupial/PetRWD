...
`AppRouter` використовує структуру **Pathless Layout Route**. Це означає, що компонент `AppShell` огортає всі сторінки, не маючи власного префікса в URL. 

**Роль AppRouter у композиції:**
`AppRouter` не просто визначає шляхи, він збирає каркас додатку. Він імпортує віджети (`Header`, `Sidebar`, `Footer`) та фічі (наприклад, `ThemeSwitcher`) і розставляє їх по відповідних слотах компонента `AppShell`. Це дозволяє тримати `AppShell` чистим візуальним контейнером, а сторінки — незалежними одиницями.
